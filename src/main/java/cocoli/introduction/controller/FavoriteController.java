...
import cocoli.introduction.entity.response.request.FavoriteRequestBody;
import cocoli.introduction.service.FavoriteService;
...
public class FavoriteController {
    ...
    public void unsetFavoriteItem(@RequestBody FavoriteRequestBody requestBody, HttpServletRequest request, HttpServletResponse response) {
        HttpSession session = request.getSession(false);  //if i cannot find session, return false
        if (session == null) {
            response.setStatus(HttpServletResponse.SC_FORBIDDEN);
            return;
        }
        String userId = (String) session.getAttribute("user_id");  //convert to string
